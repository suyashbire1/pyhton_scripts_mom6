...
#                slat=ystart, nlat=yend,zs=zs,ze=ze)
        ...
        hfv = fh.variables['twa_hfv'][0:,zs:ze,ys:ye,xs:xe]
        hpfu = fh.variables['twa_hpfu'][0:,zs:ze,ys:ye,xs:xe]
        huuxpt = fh.variables['twa_huuxpt'][0:,zs:ze,ys:ye,xs:xe]
        huvymt = fh.variables['twa_huvymt'][0:,zs:ze,ys:ye,xs:xe]
        huwb = fh.variables['twa_huwb'][0:,zs:ze,ys:ye,xs:xe]
        hdudtvisc = fh.variables['twa_hdudtvisc'][0:,zs:ze,ys:ye,xs:xe]
        hdiffu = fh.variables['twa_hdiffu'][0:,zs:ze,ys:ye,xs:xe]
            
        fh.close()

        terms = np.ma.concatenate(( hfv[:,:,:,:,np.newaxis],
                                    hpfu[:,:,:,:,np.newaxis],
                                    huwb[:,:,:,:,np.newaxis],
                                    huuxpt[:,:,:,:,np.newaxis],
                                    huvymt[:,:,:,:,np.newaxis],
                                    hdudtvisc[:,:,:,:,np.newaxis],
                                    hdiffu[:,:,:,:,np.newaxis]),
                                    axis=4)
