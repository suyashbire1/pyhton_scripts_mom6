...
def extract_twamomy_terms(geofil,vgeofil,fil,xstart,xend,ystart,yend,zs,ze,meanax,
    ...
    if not alreadysaved:
        ...
        hforydiff = fh.variables['h'][0:1,zs:ze,ys:ye+1,xs:xe]
        v = fh.variables['v'][0:1,zs:ze,ys:ye,xs:xe]
        nt = np.ones(v.shape)*nt_const
        ntforydiff = np.ones(hforydiff.shape)*nt_const
        frhatv = fh.variables['frhatv'][0:1,zs:ze,ys:ye,xs:xe]
        h_v = frhatv*D[np.newaxis,np.newaxis,:,:]
        h_v = np.ma.masked_array(h_v,mask=(h<=1e-3).astype(int))
        nt[h<=1e-3] -= 1
        ntforydiff[hforydiff<=1e-3] -= 1
        hvm = (h_v*v).filled(0)
        ...
        pfv = np.ma.masked_array(pfv,mask=(h<=1e-3).astype(int))
        pfvm = pfv.filled(0)
        ...
        hwm_v = np.ma.masked_array(hwm_v,mask=(h<=1e-3).astype(int))
        hwm_v = hwm_v.filled(0)

        eforydiff = fh.variables['e'][0:1,zs:ze,ys:ye+1,xs:xe]
        e = fh.variables['e'][0:1,zs:ze,ys:ye,xs:xe]
        em = e/nt_const
        el = 0.5*(eforydiff[:,:-1,:,:] + eforydiff[:,1:,:,:])
        el = np.ma.masked_array(el,mask=(hforydiff<=1e-3).astype(int))
        elm = el.filled(0)
        ...
        for i in range(1,nt_const):
            ...
            hw = wd*dbi[:,np.newaxis,np.newaxis]
            hw = 0.5*(hw[:,0:-1,:,:] + hw[:,1:,:,:])
            hw_v = 0.5*(hw[:,:,0:-1,:] + hw[:,:,1:,:])
            hw_v = np.ma.masked_array(hw_v,mask=(h<=1e-3).astype(int))
            hwm_v += hw_v.filled(0)

            eforydiff = fh.variables['e'][i:i+1,zs:ze,ys:ye+1,xs:xe]
        ...
        edlsqm = esqm/ntforydiff - (elm/ntforydiff)**2
        edlsqmy = np.diff(edlsqm,axis=2)/dycv*nt

        edpfvdm = (epfvm/nt - elmatv/nt*pfvm/nt)*nt
        edpfvdmb = np.diff(edpfvdm,axis=1)
#        edpfvdmb = np.diff(edpfvdm,axis=1)/db[1:,np.newaxis,np.newaxis]
